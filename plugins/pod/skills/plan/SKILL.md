...
(`/pod:launch`), so it must be self-contained: an executor sees the plan and
...
- **Starting the work in a lane** ⇒ `/pod:launch` (or `--launch` here, which
  invokes it).
- **Driving the Orca app** ⇒ Orca's bundled `orca-cli` skill.
- **Checking finished work** ⇒ `/pod:verify`.

## 0. Preconditions

- `$ARGUMENTS` empty ⇒ ask what to plan.
- Check for `--launch`: plan, review, then launch the lane via `/pod:launch` (§6).
- Check for `--auto`: plan and review **unattended** — never ask, never enter
  ...
  what `/pod:wave --auto` and `/pod:tech-lead` send into a terminal nobody is
...
Use exactly these sections — they are what `/pod:launch` packages:
...
**Inside a plain wave** (`/pod:wave` started this context in its own terminal,
...
blocked wave; `/pod:tech-lead` reads the finished plan file and routes the
...
- **An existing `### Acceptance criteria` section is not yours to replace.** It was written
  before this planning pass — by `/pod:triage` with the user, or by an earlier
...
context closes. `/pod:wave --review` reads these files to check plans against
each other, and `/pod:launch` can be pointed at one directly. **Never overwrite
...
`/pod:tech-lead` and `/pod:wave` read this line rather than guessing at a
...
- Close by noting `/pod:launch` can start it in its own lane — but do
...
1. **File an issue per part.** Each gets its own `### Acceptance criteria` checklist,
   covering only that part. `/pod:triage <n>` is the natural way, or file them
...
2. **Record the order as real dependency edges** — part B blocked by part A:
   ...
   This is what makes `/pod:status` show B as blocked and stops
   `/pod:launch` starting it early. Prose ordering in a plan is invisible to
...
After folding in review findings, invoke the `pod:launch` skill via the Skill
