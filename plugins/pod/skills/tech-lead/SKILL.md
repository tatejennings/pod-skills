...
The `pod` plugin is a pipeline that stops on purpose: `/pod:plan` stops at a plan, `/pod:launch`
stops at a running lane and says *do not monitor it*, `/pod:verify` stops at a verdict,
`/pod:status` only reads. Nothing in it decides what is next, keeps lanes fed, reacts to review
...
- **One handoff, no supervision** ⇒ `/pod:launch <n>`. Do not start this loop for one issue the
  user just wants started.
- **Planning one issue with the user in the loop** ⇒ `/pod:plan <n>`.
- **Planning several with the user answering each context's questions** ⇒ `/pod:wave`.
- **A read-only answer to "what's ready / how are the lanes"** ⇒ `/pod:status`. This is the
  closest neighbour and the easiest mis-route: `/pod:status` *reports*, this skill *acts*. If the
  user only wants to see the board, they want `/pod:status`, and this skill calls it anyway
  (§3.1) rather than reimplementing the join.
- **Supervising dispatch workers** with `worker_done` / `escalation` waits ⇒ Orca's bundled
  `orchestration` skill. Lanes launched by `/pod:launch` are contract-driven, not dispatches — they
...
Then confirm the pipeline this skill composes is present: `/pod:status`, `/pod:plan`,
`/pod:launch` must be invocable (`pod` plugin installed). Missing ⇒ stop and name the install
command from the plugin README. Confirm the repo is on the tracking model — issues carry
`### Acceptance criteria`. If they do not, this is a `/pod:migrate` or `/pod:triage` job first; say so.
...
Invoke `/pod:status` (the Skill tool). The lane × backlog join is already built; **reuse it, do
...
**Re-derive, never recall.** Everything in this step comes from `/pod:status` and `gh` this tick.
...
- **`gated-stale`** ⇒ a verdict exists but commits landed after it ran, so nothing has checked the
  current tree. **Re-gate it** with `/pod:verify <n>` — this is the one gate action you take on
...
- **Human-review-ready** ⇒ mark it in the ledger, push-notify once per PR, and do not touch it
  ...
  | all required CI checks **succeeded**, none pending | `statusCheckRollup` from `/pod:status`. The gate checks the issue's criteria; CI checks the repo's. They are different questions |
...
1. **Pick** — the scope's own order first (the slate the user approved), then smallest `scope:*`
   ...
   `### Acceptance criteria` (say so — it needs `/pod:triage` or `/pod:plan` first, and `/pod:plan` is
   the next step anyway, so it will get one).

   **A criteria-less issue is a hard skip here, not an offer.** `/pod:launch` will hand off
...
2. **Plan with experts** — `references/expert-panel.md`. Planning itself runs in a separate Orca
   ...
   **Write the ledger row before you start it** (`#<n> | planning (terminal <handle>)`), not at the
   ...
   orca terminal send --terminal <handle> --text "/pod:plan <n> --auto" --enter --json
   orca terminal wait --terminal <handle> --for tui-idle --timeout-ms 900000 --json
   ```

   `--auto` is what makes that terminal safe to leave alone: `/pod:plan` §0 reads it as *never
   ...
   **Re-resolve the handle before each command** (`orca terminal list --worktree active --json`).
   ...
   Then take the plan file from `/pod:plan`'s last line — it prints `PLAN FILE: <absolute path>`
   ...
   - **The tail shows an unrecognized-flag stop** ⇒ `/pod:plan` is older than this skill. Say so
   ...
   **Close the terminal on every one of those paths**, not just the first — `orca terminal close
   ...
   where `/pod:status` cannot see it (it filters `isMainWorktree == false`), so leaks are
   ...
   excludes it by construction). It goes to *Waiting on you*, naming `/pod:plan` §5a as the next
   step: each part needs its own issue with its own `### Acceptance criteria` before anything launches.
3. **Launch** — invoke `/pod:launch <n>` **in your own context**, with the reviewed plan already
   in it. `/pod:launch` fills the contract from the issue and the plan it can see, runs its own
...
- `ScheduleWakeup`: ~10 minutes while a plan or a review-fix is in flight, 20–30 minutes otherwise,
  `noop: true` when nothing changed. Pass the same `/pod:tech-lead …` words back as the prompt.
...
**Restart is always cheap:** `/pod:tech-lead resume`, `just go`, or any imperative restarts from the
...
- **A second lane on one issue.** Check in-flight yourself *and* let `/pod:launch` check again.
  Two nets are right.
