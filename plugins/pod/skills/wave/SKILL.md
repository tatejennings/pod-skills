...
edit the same file — `/pod:status` cannot see that, because it only knows
...
- **Planning one issue** ⇒ `/pod:plan`. This is the fan-out wrapper.
- **Creating worktrees** ⇒ it deliberately does not. Planning writes no repo
  ...
  requirement"). Only `/pod:launch` creates a worktree.
...
`/pod:plan` still works for one issue at a time.
...
- **No `### Acceptance criteria` checklist** — plannable, but say so: the plan will have to
  establish criteria, and `/pod:triage` is the cheaper place to do that.
...
The mechanism already exists: send `/pod:plan <n> --auto` instead of
`/pod:plan <n>`. That mode holds a deliberately high bar for deciding alone —
...
Answer #86, then: /pod:wave --review 84 85 86 87
```
...
orca terminal send --terminal <handle> \
  --text "/pod:plan <n>" --enter --json
```

**Under `--auto`, send `/pod:plan <n> --auto`** — that flag is what makes the
...
- **Send `/pod:plan <n>`, not a paraphrase.** Each context runs the real
...
/pod:wave --review 84 85 86 87 to check the plans against each other.
```
...
`/pod:plan` writes to `~/.claude/plans/<repo-name>/`, and **that directory
...
1. **Issue numbers in `$ARGUMENTS`** — `/pod:wave --review 84 85 86 87`. The
...
the `pod:launch` skill via the Skill tool with that issue number.
...
`/pod:launch` refuses in-flight work itself, so this is a second net rather than
