...
- the PR is **ungated** (`/pod:status` shows `awaiting-gate`) — the lane skipped
...
- **Merging, or marking a PR ready** ⇒ never. `pass` is a report; a human acts on
  it. See §6.
- **Deciding what the criteria should be** ⇒ `/pod:plan` writes them,
  `/pod:migrate` retrofits them. This skill only checks.
...
them is the exact failure this gate exists to prevent. Point at `/pod:plan` or
`/pod:migrate` to establish them.
...
- **`fail`** ⇒ comment the unmet criteria and their evidence
  ...
  **Then name the next action, because a comment is not one.** Offer
  `/pod:launch <n>`, which has a rework path (§1a there): it reuses the existing
...
- **Any passing verdict** ⇒ report it, **and post the verdict as a PR comment.**
  ...
  `/pod:status` cannot tell a gated PR from an ungated one — they look
  identical.
...
from one a later push invalidated, so `/pod:status` treats it as `gated-stale`
