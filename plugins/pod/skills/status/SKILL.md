...
- **Deciding how to do a piece of work** ⇒ `/pod:plan`.
...
is read-only, which is what keeps `/loop 15m /pod:status` safe.
...
- **`statusCheckRollup`** ⇒ CI state. **This gate is not CI**, and a `pass` from
  `/pod:verify` says nothing about it — showing both side by side is what stops
...
| `awaiting-gate` | a PR is open, no agent is active, and **no gate verdict is on it** — the lane's self-gate did not run; gate it with `/pod:verify` |
| `gated-stale` | a valid verdict exists but its `head=` is not the PR's current `headRefOid` (or the base moved under it) — **commits landed after the gate ran.** Not a failure: nobody has checked this tree. Re-gate with `/pod:verify` |
...
- **Flag issues with no acceptance checklist under either heading** (`../_shared/issue-schema.md`) — they cannot be gated by
  `/pod:verify`, so mark them `(no criteria)` and point at `/pod:plan` or
  `/pod:migrate`.
- Nothing ready and nothing blocked ⇒ one line pointing at `/pod:migrate`
  (audit the backlog) or `/pod:plan` (nothing is filed yet).
...
- `awaiting-gate` → "PR open but **never gated** — the lane's self-gate did not
  run. Gate it with `/pod:verify <n>` before merging." Say *never gated* rather
...
- `gated-stale` → **"gated, then more commits landed — the current code is
  ...
  the gap is visible. Next action: `/pod:verify <n>` re-gates it. Say
...
- `gate-failed` → **"its own gate rejected this branch — do not merge"**, naming
  ...
  `/pod:launch <n>` reworks it in the existing lane (§1a there), carrying the
...
- **failing `statusCheckRollup`** → "CI failing", stated alongside the gate
  ...
  `/pod:verify` checks the issue's criteria against the branch; it is not CI and
  does not supersede it.
- `merged-reapable` → "run `/pod:status --reap` to clean it up" (or reap now if
  `--reap` was passed)
...
`/pod:migrate`, which proposes untracking it with consent. This guard matters
...
<!-- GENERATED by /pod:status — do not edit; it is rewritten on every run -->
<!-- <YYYY-MM-DD HH:MM> -->
...
makes `/loop 15m /pod:status --reap` safe.
