...
- **Migrating a repo's whole tracking model** (files, conventions, the
  `AGENTS.md` block, the schema version) ⇒ `/pod:migrate`. Both skills audit,
  at different altitudes: `/pod:migrate` checks **repo-level structure** — is
...
- **Planning how to implement an issue** ⇒ `/pod:plan`. Triage decides *whether
  and when*; planning decides *how*.
- **Deciding what to work on now** ⇒ `/pod:status`.
...
issues, say so and offer `/pod:migrate` first. This skill assumes milestones and
...
Use this when the user names specific issues (`/pod:triage 101 103 107`), or
...
- **Can an agent do this at all?** Ask when it is not obvious. Work behind
  ...
  so `/pod:launch` refuses to launch it and `/pod:status` lists it under YOUR
...
- **What is now ready to plan** — anything that came out of triage with a
  milestone, criteria, and no open blockers is `/pod:plan` input, and saying so
  closes the loop.
