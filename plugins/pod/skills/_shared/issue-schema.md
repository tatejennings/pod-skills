...
One shape, seven consumers. `/pod:migrate` establishes it, `/pod:triage` and
`/pod:plan` write it,
`/pod:launch` binds an executor to it, `/pod:status` reads readiness from it,
and `/pod:verify` mechanically checks it. **Point at this file; do not restate
...
- **`### Acceptance criteria`** — the checklist that gates the work. Required,
  ...
  > `### Acceptance criteria` for anything new**; `/pod:migrate` rewrites the old
  ...
  > which is the one state that disables the gate and makes `/pod:launch` refuse
...
`/pod:verify` sorts every checklist item into one of three buckets. The bucket
...
machine cannot check it. `/pod:verify` must **never** guess at these, never mark
...
- **Named for the area, not the letter.** `sound` beats `workstream:E` — a
  reader should not need a legend, and these surface in `/pod:status`.
...
- **`/pod:status`** lists `manual` issues in their own section rather than under
...
- **`/pod:launch`** refuses to launch a lane for one and says why. This is the
  payoff: it stops an agent being handed something it structurally cannot finish.
- **`/pod:triage`** asks when it is not obvious from the issue.
...
- `### Done when` — the former name, still valid, rewritten by `/pod:migrate`
...
- `/pod:verify` has nothing to do without an `### Acceptance criteria` section. Missing ⇒
...
- `/pod:status` reads readiness from milestones and dependency edges, not from
  the checklist. It works on any repo.
- `/pod:launch` binds the executor to the checklist when one exists, and hands
  off fine when it does not — it notes the absence in the contract.
- `/pod:triage` **writes** the checklist by asking the user what "done" looks
  like. This is the main ongoing source of conforming issues.
- `/pod:plan` **writes** the checklist when it drafts or refines an issue. This
  is the main way conforming issues come to exist after adoption.

`/pod:migrate` is the only skill that retrofits the schema across a backlog, and
