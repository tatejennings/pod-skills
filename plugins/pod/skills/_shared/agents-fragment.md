...
`/pod:migrate` appends the block below to a consuming project's `AGENTS.md` or
...
`/pod:migrate` reads it on every later run to tell a repo migrated under an
...
requirement the repo's existing issues already meet. `/pod:migrate` rewrites the
