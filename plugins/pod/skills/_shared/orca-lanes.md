...
This is why `/pod:status` is cheap: Orca stores lane state, so the lane half is
...
- `/pod:status` maps a lane to its work through `linkedIssue` — a single
  integer, so a worktree serving two issues can only report one.
- `/pod:verify` gates a branch against **one** issue's criteria.
...
than half-working. The backlog half of some skills (`/pod:status`'s milestone
and readiness sections, all of `/pod:migrate`) is pure `gh` and still works —
